...
class ABC_Model(ABC):
    ...
    def global_max_pooling_2d(self, x, keepdims=False):
        x = torch.max(x.view(x.size(0), x.size(1), -1), -1)[0]
        if keepdims:
            x = x.view(x.size(0), x.size(1), 1, 1)
        return x

    def global_max_avg_pooling_2d(self, x, keepdims=False):
        max_p = self.global_max_pooling_2d(x, keepdims)
        avg_p = self.global_average_pooling_2d(x, keepdims)
        return max_p + avg_p
