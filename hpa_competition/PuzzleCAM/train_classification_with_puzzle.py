...
from hpa_competition.PuzzleCAM.core.puzzle_utils import *
...
# import hpa_competition.model_training.common.datasets
...
def load_RGBY_image(root_path, image_id, train_or_test='train', image_size=None):
    ...
    # print(np.array([red, green, blue]).shape)
    rgb_arr = np.array([red, green, blue])
    # stacked_images = np.transpose(rgb_arr, (1, 2, 0))
...
class HPADatasetCAM(Dataset):
    ...
    def __init__(self, path, df, img_size=224, transform=None):
        self.NUM_CL = 19
        self.path = path
    ...
    def __getitem__(self, index):
        ...
        # y = np.eye(self.NUM_CL, dtype='float')[y]
        # y = y.sum(axis=0)
        return X.type(torch.FloatTensor), torch.from_numpy(y).type(torch.FloatTensor)
...
# CUDA_VISIBLE_DEVICES=1 python3 train_classification_with_puzzle.py  --tag ResNeSt101@Puzzle@optimal --data_dir
###############################################################################
# Network
###############################################################################
...
if __name__ == '__main__':
    ...
    tensorboard_dir = create_directory(
        f'../experiments/tensorboards/{args.tag}-{datetime.now().strftime("%Y-%m-%d-%H-%M-%S")}/')
    ...
    # normalize_fn = Normalize(None, None)
    ...
    # TODO: Rewrite datasets
    ds_path = '/datasets/kolinko/hpa'
    ...
    train_dataset = HPADatasetCAM(ds_path, df, img_size=224, transform=train_transform)
    ...
    # Network
    ###################################################################################
    model = Classifier(args.architecture, num_classes=19, mode=args.mode)
